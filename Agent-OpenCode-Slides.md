...
## 多智能体辩论（Multi-Agent Debate Skill）

### 它是什么

`debate` Skill 是 OpenCode 内置的**结构化多智能体辩论协议**：用户提出命题（如「这份公告对股价是正向还是负向影响？」），系统自动编排**正方 (Pro)** 和**反方 (Con)** 进行 4 轮辩论，最后**中立裁判 (Judge)** 裁定胜负。全部内容实时写入 `debate.md`。

### 五阶段工作流

```
立论 (Opening) → 反驳 1 (Rebuttal 1) → 反驳 2 (Rebuttal 2) → 总结 (Closing) → 裁决 (Verdict)
```

| 阶段 | 正反方 | 模型 | 字数限制 |
|------|--------|------|----------|
| **立论** (Round 0) | Pro + Con **并行** | `ultrabrain` × 2 | 200–400 词 |
| **反驳 1** (Round 1) | 针对对方立论，**并行** | `ultrabrain` × 2 | 150–300 词 |
| **反驳 2** (Round 2) | 针对对方反驳，**并行** | `ultrabrain` × 2 | 150–300 词 |
| **总结** (Round 3) | 汇总最强论点，**并行** | `ultrabrain` × 2 | 150–250 词 |
| **裁决** | Judge 单独执行 | `oracle` | 200–400 词 |
...
| 角色 | 模型 | 关键设计 |
|------|------|----------|
| **Pro**（正方） | `ultrabrain` | 高推理强度，构建正面论证 |
| **Con**（反方） | `ultrabrain` | **与 Pro 同模型** → 保证公平 |
| **Judge**（裁判） | `oracle` | 只读、高智商推理，中立评估论证质量 |

- 每轮 Pro 和 Con **同时启动**（`run_in_background=true`），互不等待
- 通过 `session_id` 保持同一角色跨轮次的上下文连续性，避免重复读文件
- 裁判评估维度：论证强度、反驳质量、证据质量、逻辑一致性、修辞效果

> 触发词：`/debate`、`辩论`、`multi-agent debate`、`AI debate`
...
来自 `Multi-Agent-Debate/TODO.md`：

> 文件 `603007_1225369920_丹阳顺景智能科技股份有限公司2026年度向特定对象发行A股股票预案.txt` 是一份上市公司的公告。请根据以上公告的内容，开展多智能体辩论，辩论的议题是：**这份公告对股票价格是正向影响还是负向影响？** 请最终裁决辩论胜利方（正向/负向），也可以给出「平局」的裁决。所有辩论过程均保存到文件中。

**实际执行方式**：用户输入「读取 TODO.md 并执行」，Sisyphus 自动加载 `debate` Skill，读取公告全文（约 800+ 行），严格按照协议展开 4 轮辩论 → 裁决。

### 辩论背景

顺景科技（603007）发布 2026 年度定向增发预案：拟向 ≤35 名特定对象发行 ≤30% 股本（≤2.65 亿股），募资 ≤10.77 亿，投向压力传感器生产线（2.12 亿）、IMU 生产线（3.98 亿）、研发中心（3.07 亿）和补充流动资金（1.6 亿）。公司此前从园林工程 → 金属油箱，正谋求进入汽车传感器领域。

[对话历史](https://opncd.ai/share/XwBy5CbF)
...
| 论点 | 核心逻辑 |
|------|----------|
| **战略转型高增长市场** | 中国汽车传感器市场 982 亿 + IMU 全球 8.2% CAGR；国产替代政策背书；从低端制造向高端精密制造跃升 |
| **大股东 18 个月锁仓 = 强烈信号** | 控股股东苏州辰顺锁仓期（18 个月）是其他对象（6 个月）的 3 倍 → 用真金白银和时间赌股价上涨 |
| **现有客户 = 现成需求通道** | 已向理想、零跑、奇瑞、岚图、上汽大通供货油箱 → 交叉销售传感器大幅降低商业化摩擦 |

### 反方 (Con) — 公告对股价有负向影响

| 论点 | 核心逻辑 |
|------|----------|
| **毁灭性稀释，无近期回报** | ≤30% EPS 稀释 + 36 个月建设期 + 零 MEMS 经验 → 投资者当下买单，回报遥不可及 |
| **控股股东是空壳，不是信号** | 苏州辰顺 2025 年收入为 0、净亏 571 万 → 亏损空壳参与 10 亿增发是红旗；18 个月锁仓是监管硬性要求，不是自愿信号 |
| **三次行业跳跃，零信用积累** | 园林工程 → 金属油箱 → MEMS 传感器，三个毫无交集的行业；无半导体 IP、无制造经验、尚无备案环评 |
...
- **反方最强点**：投资者当前承受≤30% 的潜在稀释，而项目需数年才可能产生回报，且公司正进入技术门槛极高的品类但无验证能力。Con 成功将「锁仓是监管要求」这一事实逻辑嵌入驳回 Pro 的核心「内部人信心」论点。
- **正方最强点**：已有的汽车 OEM 关系确实可降低商业化摩擦，从低附加值业务向汽车电子转型的方向本身有说服力。但过度依赖 TAM 规模和政策叙事，未能充分解释为什么**顺景科技这家公司**能在合理时间内抓住机会。
- **辩论转折**：Round 2 Con 的反驳将「锁仓 = 信号」成功解构为「监管要求 ≠ 自愿信心」，直击 Pro 论证基础。

### 关键经验

1. **多 Agent 协作范式**：不是单个模型多次推理，而是**两个对等 Agent + 一个裁判 Agent**的真人辩论模拟
2. **对称模型 + session_id 保障公平**：Pro 和 Con 同模型、同上下文窗口，消除系统性偏差
3. **Oracle 裁判能识别「空洞」**：不会因语言自信而加分，会察觉「用市场总规模替代公司前景」等逻辑漏洞
4. **从单模型分析到多模型辩证**：同一篇公告，单模型情绪分析只能给标签；多智能体辩论能揭示分歧背后的深层逻辑
...
[对话记录](https://opncd.ai/share/fVvzui3k)
...
[对话记录](https://opncd.ai/share/LUK7cQtj)
